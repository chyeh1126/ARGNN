...
class Dataset():
    ...
    def __init__(self, root, name, val_rate, test_rate, setting='nettack', seed=None, require_mask=False):
        self.name = name.lower()
        self.setting = setting.lower()

        assert self.name in ['cora', 'citeseer', 'cora_ml', 'polblogs', 'pubmed',
                             'texas', 'cornell', 'wisconsin', 'chameleon', 'film', 'squirrel',
                             'acm', 'flickr', 'uai', 'amazon_photo'], \
        ...
        if self.name in ["acm", "flickr", "uai"]:
            self.data_folder = osp.join(self.data_folder, self.name)
        else:
        ...
        self.val_rate, self.test_rate = val_rate, test_rate
    ...
    def get_train_val_test(self):
        ...
        if self.setting == 'nettack':
            return get_train_val_test(nnodes=self.adj.shape[0], val_size=self.val_rate, test_size=self.test_rate, stratify=self.labels, seed=self.seed)
        if self.setting == 'gcn':
    ...
    def load_acm(self):
        with open(self.data_folder + ".edge") as f:
            lines = f.readlines()
            total_nodes = 0
            labels = []
            for line in lines:
                labels.append(line.strip().split(" "))
                for node_idx in line.strip().split(" "):
                    if int(node_idx) >= total_nodes:
                        total_nodes = int(node_idx) + 1
            adj = np.zeros((total_nodes, total_nodes))
            for i in range(len(labels)):
                adj[int(labels[i][0])][int(labels[i][1])] = 1
            adj = sp.csr_matrix(adj)
            
        with open(self.data_folder + ".feature") as f:
            lines = f.readlines()
            feature = []
            for line in lines:
        ...
        with open(self.data_folder + ".label") as f:
            lines = f.readlines()
            label = []
            for line in lines:
    ...
    def get_adj(self):
        # if self.name == "acm":
        #     adj, features, labels = self.load_acm()
        # else:
    ...
    def load_npz(self, file_name, is_sparse=True):
        if self.name in ["cora", 'cora_ml', "citeseer", "polblogs"]:
            with np.load(file_name) as loader:
                ...
                if features is None:
                    ...
                    '''random generate feature matrix'''
                    np.random.seed(42)
                    features = np.random.randint(low=0, high=2, size=adj.shape)
            features = sp.csr_matrix(features, dtype=np.float32)

        if self.name in ["cornell", "texas", "wisconsin", "chameleon", 'film', 'squirrel', 'amazon_photo']:
            data = np.load(file_name, allow_pickle=True)
            adj, features, labels = data["adj"], data["feature"], data["label"]
            adj = sp.csr_matrix(adj)
...
def get_Ptbfeature(root, name, attack_method, ptb_rate):
    if attack_method == 'mettack' or attack_method == 'metattack':
        attack_method = 'meta'
    name = name.lower()
    data_filename = osp.join(root,
                '{}_{}_feature_{}.npz'.format(name, attack_method, ptb_rate))

    feature = sp.load_npz(data_filename)
