...
parser.add_argument('--number', type=str, help="the number of graph in graph classification dataset")
...
# record the error
# logging.basicConfig(filename="/home/chy1024/MLG_FinalProject/generate_meta_aids.log", level=logging.DEBUG,
#                    format="%(asctime)s %(levelname)s %(name)s, %(message)s")
# logger = logging.getLogger(__name__)
root = './data/'
data = Dataset(root=root, name=args.dataset, setting='nettack', val_rate=args.val_rate, test_rate=args.test_rate)
...
if args.dataset == "amazon_photo":
    ...
    perturbations = int(args.ptb_rate * (adj.sum()//2) * 0.2)
...
def main():
    ...
    test(modified_adj)
    print(torch.equal(modified_features,features.to(device)))

    # if you want to save the modified adj/features, uncomment the code below
    import os
    if args.dataset in ["AIDS"]:
        root = os.path.join("./data/{}_{}/{}/{}/attack".format(args.val_rate * 100, args.test_rate * 100, args.label_rate,
                                                       args.dataset))
    else:
        root = os.path.join("./data/{}_{}/{}".format(args.val_rate * 100, args.test_rate * 100, args.label_rate))
    if not os.path.exists(root):
        os.makedirs(root)
    model.save_adj(root=root, name='{}_meta_adj_{}'.format(args.dataset ,args.ptb_rate))
    model.save_features(root=root, name='{}_meta_feature_{}'.format(args.dataset ,args.ptb_rate))
